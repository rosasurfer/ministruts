...
class DNSResolver
{
     ...
     *
     * @var string $dotfile
     * @see DNSResolver::$confpath
     */
    protected $dotfile = '.resolv.conf';
    /**
     * A array of directories to search for the user's resolver config
     *
     * @var string $confpath
     * @see DNSResolver::$dotfile
     */
    protected $confpath;
    /**
     * debugging flag
     ...
     *
     * If set to true (non-zero), the DNSResolver will attempt to use the
     * much more effecient PHP sockets extension (if available).
     *
     * @var boolean $useEnhancedSockets;
     * @access public
     */
    protected $useEnhancedSockets = true;
    /**
    ...
    /**
    /* }}} */
    /* class constructor - DNSResolver() {{{ */
    /**
     * Initializes the DNSResolver Object
     *
     * @return DNSResolver
     */
    function DNSResolver($defaults = array())
    {
        $mydefaults = array(
                'nameservers' => array(),
                'port'        => '53',
     ...
     *
     * @see DNSResolver::$resolv_conf, DNSResolver::$dotfile,
     *      DNSResolver::$confpath, DNSResolver::$searchlist,
     *      DNSResolver::$domain, DNSResolver::$nameservers
     * @access public
     */
    function res_init()
    {
     ...
     * the default settings of a new DNSResolver.
     *
     * @access public
     */
    function nameservers($nsa = array())
    {
        $defres = new DNSResolver();
    ...
    /* }}} */
    /* not tested -- DNSResolver::cname_addr() {{{ */
        ...
        foreach ($packet->answer as $rr) {
            if (in_array($rr->name, $names)) {
                if ($rr->type == 'CNAME') {
                    ...
                    //next DNSResourceRecord unless $rr->address =~ m/^($oct2\.$oct2\.$oct2\.$oct2)$/o;
     ...
     * @return mixed    an object of type DNSPacket on success,
     *                  or false on failure.
     ...
     * @param string $type The type of record to query.
     * @param string $class The class of record to query.
     * @return mixed an object of type DNSPacket, regardless of whether the packet
    ...
    function rawQuery($name, $type = 'A', $class = 'IN')
    {
        ...
        $packet = new DNSPacket($this->debug);
    ...
    /* }}} */
    /* DNSResolver::send($packetORname, $qtype = '', $qclass = '') {{{ */
    /**
     ...
     * query here and bypass DNSResolver::query()
     *
     ...
     * @return object DNSPacket    An answer packet object
     */
    ...
    /* }}} */
    /* DNSResolver::printhex($packet_data) {{{ */
    /**
    ...
    /* }}} */
    /* DNSResolver::send_tcp($packet, $packet_data) {{{ */
    /**
     ...
     *                              the DNSPacket::data() method
     * @return object DNSPacket Returns an answer packet object
     * @see DNSResolver::send_udp(), DNSResolver::send()
     */
    ...
    /* }}} */
    /* DNSResolver::send_udp_no_sock_lib($packet, $packet_data) {{{ */
    /**
     ...
     *                              the DNSPacket::data() method
     * @return object DNSPacket Returns an answer packet object
     * @see DNSResolver::send_tcp(), DNSResolver::send(),
     *      DNSResolver::send_udp(), DNSResolver::send_udp_with_sock_lib()
     */
    ...
    /* }}} */
    /* DNSResolver::send_udp_with_sock_lib($packet, $packet_data) {{{ */
    /**
     * Sends a packet via UDP to the list of name servers.
     *
     * This function sends a packet to a nameserver.  It is called by
     ...
     * @return object DNSPacket Returns an answer packet object
     * @see DNSResolver::send_tcp(), DNSResolver::send(),
     *      DNSResolver::send_udp(), DNSResolver::send_udp_no_sock_lib()
     */
    ...
    /* }}} */
    /* DNSResolver::send_udp($packet, $packet_data) {{{ */
    /**
     * Sends a packet via UDP to the list of name servers.
     *
     * This function sends a packet to a nameserver.  send_udp calls
     * either DNSResolver::send_udp_no_sock_lib() or
     * DNSResolver::send_udp_with_sock_lib() depending on whether or
     * not the sockets extension is compiled into PHP.  Note that using the
     * sockets extension is MUCH more efficient.
     *
     * @param object DNSPacket $packet A packet object to send to the NS list
     * @param string $packet_data   The data in the packet as returned by
     *                              the DNSPacket::data() method
     * @return object DNSPacket Returns an answer packet object
     * @see DNSResolver::send_tcp(), DNSResolver::send(),
    ...
    /* }}} */
    /* DNSResolver::make_query_packet($packetORname, $type = '', $class = '') {{{ */
    /**
     * Unknown
     */
    ...
    /* }}} */
    /* DNSResolver::axfr_old($dname, $class = 'IN') {{{ */
    /**
    ...
    /* }}} */
    /* DNSResolver::axfr($dname, $class = 'IN', $old = false) {{{ */
    /**
     ...
     * DNSResolver::$usevc flag.  If $old is set to true, DNSUtil requires
    ...
    /* }}} */
    /* DNSResolver::axfr_start($dname, $class = 'IN') {{{ */
    /**
     * Sends a packet via TCP to the list of name servers.
     *
     * @param string $packet    A packet object to send to the NS list
    ...
    /* }}} */
    /* DNSResolver::axfr_next() {{{ */
    /**
     * Requests the next DNSResourceRecord from a existing transfer started with axfr_start
     *
     * @return object DNSResourceRecord Returns a DNSResourceRecord object of the next DNSResourceRecord
