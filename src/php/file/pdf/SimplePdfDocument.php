 ...
 *
 * This class will take the basic interaction facilities of the BasePdfDocument
 * class and make more useful functions so that the user does not have to
...
class SimplePdfDocument extends BasePdfDocument {
   ...
   function SimplePdfDocument($paper='a4', $orientation='portrait') {
      ...
      if (!is_array($paper)) {
         ...
         if (strToLower($orientation) == 'landscape') { // swap dimensions
               $tmp     = $size[2];
               $size[2] = $size[3];
               $size[3] = $tmp;
         }
         ...
         if (sizeOf($paper) > 2) {  // an array was sent to set the size
            $size = $paper;
         }
