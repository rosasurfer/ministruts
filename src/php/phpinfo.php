...
// Extensions
// ----------
if (!extension_loaded('ctype'))                                              $isWarning |= warn('Warning: ctype extension is not loaded');
...
if ( extension_loaded('apc')) {
   ...
   if (WINDOWS) {       // Entwicklungsumgebung
      if      (ini_get('apc.stat'))                                          $isWarning |= warn('Warning: apc.stat is not Off');                  // "On" läßt manche Windows-APC-Versionen crashen (apc-error: cannot redeclare class ***)
      ...
      if ( ini_get('apc.stat'))                                              $isWarning |= warn('Warning: apc.stat is not Off');                  // für beste Performance möglichst "Off" (Dateiänderungen sollen live nicht möglich sein)
