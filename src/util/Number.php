...
class Number extends StaticClass {
    ...
    public static function formatMoney($value, $decimals=2, $decimalsSeparator='.', $thousandsSeparator='') {
        if (!is_int($value) && !is_float($value)) throw new IllegalTypeException('Illegal type of parameter $value: '.gettype($value)); // @phpstan-ignore booleanAnd.alwaysFalse (types come from PHPDoc)
