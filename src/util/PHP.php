...
class PHP extends StaticClass {
    ...
    public static function phpinfo(): void {
        ...
        /*PHP_INI_PERDIR*/ $postMaxSize = ini_get_bytes('post_max_size');
            ...
            elseif ($localMemoryLimit < $postMaxSize + 20*MB) /*PHP needs about 20MB for the runtime*/             $issues[] = 'Warn:  local memory_limit "'.ini_get('memory_limit').'" is very low for post_max_size "'.ini_get('post_max_size').'" (PHP needs at least 20MB) [configuration]';
            elseif ($globalMemoryLimit < $postMaxSize)                                                             $issues[] = 'Warn:  make sure memory_limit "'.ini_get('memory_limit').'" is raised before script entry as global memory_limit "'.ini_get_all()['memory_limit']['global_value'].'" is too low for post_max_size "'.ini_get('post_max_size').'" [configuration]';
